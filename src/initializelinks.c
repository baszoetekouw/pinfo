...
inline int
exists_in_tag_table (char *item)	/* checks if an item belongs to
					 * tag table. returns 1 on success
					 * and 0 on failure.
...
calculate_len (char *start, char *end)	/*
					 ...
					 * `\t' as filling up to 8 chars.
					 * (i.e. at line 22 tab will increment
					 * the counter by 2 
...
initializelinks (char *line1, char *line2, int line)
{
  ...
  long initialhyperobjectcount = hyperobjectcount;	/* required to sort properly
							   the hyperlinks from
							   current line only */
  int changed;
...
******************************************************************************/
      else if ((tmp = strrchr (line1, ':')) != NULL)
	{
	  ...
	  if (((start = strchr (tmp, '(')) != NULL) && (dot != NULL) &&
	      ((end = strchr (start, ')')) != NULL))	/* we make use of sequential AND evaluation: start must not be NULL! */
	    {
